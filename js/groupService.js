...
export class GroupService {
  ...
  async createGroup({ groupName, ownerPersonalId, ownerUid }) {
    ...
    if (name.length > 7) {
      throw new Error("グループ名7文字以内にしてください");
    }
  ...
  async approveMember({ requestId, ownerUid }) {
    ...
    const { groupId, personalId, uid } = reqSnap.data();
  ...
  async leaveGroup({ groupId, personalId }) {
    ...
    await deleteDoc(
      doc(this.db, "groupMembers", memberIdOf(personalId, groupId))
    );
  }
  ...
  async deleteGroup({ groupId }) {
    ...
    for (const col of ["groupMembers", "groupJoinRequests"]) {
      const q = query(
